...
class PlayerService {
  ...
  async callTime(tablePlayerId, userId) {
    try {
      ...
      // Only add current session time if player is actively playing
      ...
      if (remainingSeconds > 0) {
        ...
        throw new Error(
          `Player must play ${remainingMinutes} more minutes before calling time`
        );
      }
      ...
      const callTimeEndsAt = new Date(
        now.getTime() + callTimeDuration * 60 * 1000
      );
  ...
  async extendCallTime(tablePlayerId, additionalMinutes = 15, userId) {
    try {
      ...
      const newEndsAt = new Date(
        currentEndsAt.getTime() + additionalMinutes * 60 * 1000
      );
    ...
    try {
      ...
      await this.logTimeEvent(
        ...
        "removed",
        {
          notes:
            reason ||
            `Played ${totalPlayedMinutes} minutes (${totalPlayedHours} hours) on Table ${player.table_number}`,
          performed_by: userId,
        }
      );
  ...
  async updatePlayerTimeStats(playerId, sessionMinutes) {
    try {
      ...
      console.log(
        `✅ Player ${playerId} stats updated: ${sessionMinutes} minutes (${sessionHours} hours)`
      );
    } catch (error) {
      ...
      for (const player of expiredPlayers) {
        try {
          await this.removePlayer(
            ...
            1,
            "Auto-removed: Call time expired"
          );
          removedPlayers.push({
            player_name: player.player_name,
          ...
          console.error(
            `Failed to auto-remove player ${player.player_name}:`,
            err
          );
        }
      }
  ...
  async transferPlayer(tablePlayerId, newTableId, newSeatNumber, userId) {
    try {
      ...
      if (seatOccupied) {
        throw new Error(
          `Seat ${newSeatNumber} is already occupied on table ${newTable.table_number}`
        );
      }
    ...
    try {
      ...
      switch (player.player_status) {
        case "playing":
          ...
          currentPlayedSeconds =
            accumulatedSeconds + Math.floor((now - seatedAt) / 1000);
          break;
        ...
        case "on_break":
          ...
          if (player.break_ends_at) {
            ...
            breakRemainingSeconds = Math.max(
              0,
              Math.floor((breakEndsAt - now) / 1000)
            );
          }
          break;
        ...
        case "call_time_active":
          if (player.call_time_ends_at) {
            ...
            callTimeRemainingSeconds = Math.floor(
              (callTimeEndsAt - now) / 1000
            );
          }
          break;
      }
      ...
      return {
        ...
        remaining_minutes: Math.floor(
          (minimumPlaySeconds - currentPlayedSeconds) / 60
        ),

        break_remaining_seconds: breakRemainingSeconds,
        break_remaining_minutes: breakRemainingSeconds
          ? Math.floor(breakRemainingSeconds / 60)
          : null,

        call_time_remaining_seconds: callTimeRemainingSeconds,
        call_time_remaining_minutes: callTimeRemainingSeconds
          ? Math.floor(callTimeRemainingSeconds / 60)
          : null,
